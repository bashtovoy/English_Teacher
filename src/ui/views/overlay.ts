...
export class OverlayView {
  ...
  private readonly events: {
    [K in keyof OverlayViewEventMap]: EventImpl<OverlayViewEventMap[K]>;
      ...
      >(),
      "click:learnMode": new EventImpl<
        OverlayViewEventMap["click:learnMode"]
      >(),
  ...
  initUIEvents() {
    ...
    this.votButton.learnButton.addEventListener(
      "pointerdown",
      (event) => {
        if (!isPrimaryActionPointer(event)) return;
        closeMenu();
        this.events["click:learnMode"].dispatch();
      },
      { signal },
    );
    this.votButton.learnButton.addEventListener(
    ...
    this.votButton.learnButton.style.touchAction = touchAction;
