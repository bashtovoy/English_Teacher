...
import { LangLearnController } from "../langLearn/LangLearnController";
import { LangLearnPanel } from "../langLearn/LangLearnPanel";
...
export class UIManager {
  ...
  langLearnController?: LangLearnController;
  langLearnPanel?: LangLearnPanel;
  ...
  private bindOverlayViewEvents() {
    ...
    overlayView
      ...
      .addEventListener("click:learnMode", async () => {
        await this.handleLearnModeBtnClick();
      })
  ...
  private async handleLearnModeBtnClick() {
    console.log("[LangLearn] Learn Mode Button Clicked.");
    if (!this.videoHandler || !this.videoHandler.videoData) {
      console.warn("[LangLearn] VideoHandler or VideoData missing.");
      return;
    }

    if (!this.videoHandler.hasActiveSource()) {
      console.warn("[LangLearn] Missing translated audio source. Start translation first.");
      this.videoHandler.notifier.send({
        text: "Сначала включите перевод видео, чтобы получить аудио-дорожку.",
    ...
    const translatedSubtitles =
      await this.prepareTranslatedSubtitlesForLearnMode();
    const transSubtitles = translatedSubtitles?.subtitles;
    if (!transSubtitles || transSubtitles.length === 0) {
      console.warn("[LangLearn] Missing translated subtitles. Please wait for translation to load.");
      this.videoHandler.notifier.send({
        text: "Переведенные субтитры еще не загружены. Подождите немного.",
        title: "🎓 Режим изучения",
        timeout: 4000
      });
      return;
    ...
    const fromLang = (this.videoHandler.videoData.detectedLanguage ??
      this.videoHandler.translateFromLang) as string;

    console.log(`[LangLearn] Starting Fetch. fromLang=${fromLang}, videoId=${this.videoHandler.videoData.videoId}`);

    let origSubtitles;
    try {
      // Try fetching subtitles for the original language
      const origSubsData = await SubtitlesProcessor.fetchSubtitles(this.videoHandler.videoData, fromLang, fromLang);
      origSubtitles = origSubsData.subtitles;
      console.log(`[LangLearn] Fetched original subtitles count: ${origSubtitles?.length}`);
    } catch (err) {
      console.error("[LangLearn] Could not fetch original subtitles", err);
    }

    // Fallback: If origSubtitles is still empty, and we have a descriptors list, try the first one
    if ((!origSubtitles || origSubtitles.length === 0) && this.videoHandler.subtitles?.length > 0) {
      console.log("[LangLearn] Falling back to first available subtitle track.");
      try {
        const firstDescriptor = this.videoHandler.subtitles[0] as any;
        const fallbackData = await SubtitlesProcessor.fetchSubtitles(firstDescriptor);
        origSubtitles = fallbackData.subtitles;
        console.log(`[LangLearn] Fallback fetched subtitles count: ${origSubtitles?.length}`);
      } catch (err) {
        console.error("[LangLearn] Fallback fetch failed", err);
      }
    }

    if (!origSubtitles || origSubtitles.length === 0) {
      console.warn("[LangLearn] No original subtitles found after all attempts.");
      return;
    }

    if (!this.langLearnController) {
      this.langLearnController = new LangLearnController(this.videoHandler);
    }

    if (this.langLearnPanel) {
      this.langLearnPanel.close();
    }

    if (this.votGlobalPortal) {
      this.langLearnPanel = new LangLearnPanel(this.langLearnController, this.votGlobalPortal);
    }

    (globalThis as any)._votUIManager = this;

    // Start learning mode with the converted data format.
    console.log("[LangLearn] Passing subtitles to Controller...");
    this.runDetached(
      this.langLearnController.start(origSubtitles, transSubtitles as any),
      "Failed to start LangLearnMode",
    );
  }

  private async prepareTranslatedSubtitlesForLearnMode() {
    if (!this.videoHandler?.videoData) {
      return null;
    }

    const cachedSubtitles = this.videoHandler.yandexSubtitles;
    if (cachedSubtitles?.subtitles?.length) {
      return cachedSubtitles;
    }

    const fromLang = (this.videoHandler.videoData.detectedLanguage ??
      this.videoHandler.translateFromLang) as string;
    const toLang = (this.videoHandler.videoData.responseLanguage ??
      this.videoHandler.translateToLang) as string;

    try {
      const fetchedSubtitles = await SubtitlesProcessor.fetchSubtitles(
        this.videoHandler.videoData,
        toLang,
        fromLang,
      );
      if (fetchedSubtitles.subtitles.length > 0) {
        this.videoHandler.yandexSubtitles = fetchedSubtitles;
        return fetchedSubtitles;
      }
    } catch (err) {
      debug.warn(
        "[LangLearn] Failed to fetch translated subtitles directly",
        err,
      );
    }
    ...
    try {
      await this.videoHandler.enableSubtitlesForCurrentLangPair();
      if (this.videoHandler.yandexSubtitles?.subtitles?.length) {
        return this.videoHandler.yandexSubtitles;
      }
    } catch (err) {
      debug.warn(
        "[LangLearn] Failed to preload subtitles via subtitles module",
        err,
      );
    }
